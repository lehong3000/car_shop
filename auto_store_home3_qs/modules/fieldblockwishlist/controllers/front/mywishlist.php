 ...
 * @since 1.5.0
 */
class FieldBlockWishListMyWishListModuleFrontController extends ModuleFrontController
{
	public $ssl = true;
	...
	public function assign()
	{
		...
		if ($this->context->customer->isLogged())
		{
				...
				if (!count($errors))
				{
					...
					if (!count($errors))
					{
						...
						Mail::Send(
							...
							'{message}' => $this->context->link->getModuleLink('fieldblockwishlist', 'view', array('token' => $wishlist->token))
							),
						...
						Tools::redirect($this->context->link->getModuleLink('fieldblockwishlist', 'mywishlist'));
					}
				}
			}
			else if ($add)
			...
			Tools::redirect('index.php?controller=authentication&back='.urlencode($this->context->link->getModuleLink('fieldblockwishlist', 'mywishlist')));
		...
		$this->setTemplate('module:fieldblockwishlist/views/templates/front/mywishlist.tpl');
	}
