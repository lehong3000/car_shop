...
<div id="search_block_left" class="block exclusive">
	<h4 class="title_block">{l s='Search' mod='fieldblocksearch'}</h4>
	<form method="get" action="{$link->getPageLink('search')|escape:'html'}" id="searchbox">
            <input type="hidden" name="controller" value="search" />
            <p class="block_content">
                {$category_html nofilter}
                    <label for="search_query_block">{l s='Search products:' mod='fieldblocksearch'}</label>
                    ...
                    <input type="submit" id="search_button" class="button_mini" value="{l s='Go' mod='fieldblocksearch'}" />
            </p>
	</form>
</div>
...
{if $tags}
    <div id="search_tags" class="search_tags">
        {foreach $tags as $tag}
            <a href="{$tag.link}" title="{$tag.tag}">{$tag.tag}</a>
        {/foreach}
    </div>
{/if}
<!-- /Block search module -->
