...
class InstallControllerHttpProcess extends InstallControllerHttp implements HttpConfigureInterface
{
	...
	public function installDatabaseTheme()
    {
		...
		$dddd=str_replace("\\","/",_PS_ROOT_DIR_);

	if (file_exists($dddd."/install/data/theme.sql")){
	    $file = fopen($dddd."/install/data/theme.sql",'r');
	    // replace
	    $str_name=fgets($file);
	    $name_site=str_replace (substr($str_name,0,stripos($str_name,"/", 1)),"",$str_name);	
	    $name_site=trim($name_site);
	    fclose($file);
	    $file = fopen($dddd."/install/data/theme.sql",'r');	
	    $data = file_get_contents($dddd."/install/data/theme.sql");
	    $bodytag = str_replace ($name_site,__PS_BASE_URI__,$data);
	    fclose($file);
	    $file = fopen($dddd."/install/data/theme.sql",'w+');
	    fwrite($file, $bodytag);
	    fclose($file);
	}
        ...
        $sql_loader->setMetaData(array(
            '`ps_' => '`'._DB_PREFIX_,
            'ENGINE=InnoDB' => 'ENGINE='._MYSQL_ENGINE_,
        ));
