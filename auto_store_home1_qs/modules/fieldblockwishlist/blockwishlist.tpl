...
<div id="wishlist_block" class="block account">
	<h4 class="title_block">
		<a href="{$link->getModuleLink('fieldblockwishlist', 'mywishlist', array(), true)|addslashes}" title="{l s='My wishlists' mod='fieldblockwishlist'}" rel="nofollow">{l s='Wishlist' mod='fieldblockwishlist'}</a>
	</h4>
	<div class="block_content">
		...
		{if $wishlist_products}
			...
			{foreach from=$wishlist_products item=product name=i}
				<dt class="{if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}">
					...
					<a class="ajax_cart_block_remove_link" href="javascript:;" onclick="javascript:WishlistCart('wishlist_block_list', 'delete', '{$product.id_product}', {$product.id_product_attribute}, '0', '{if isset($token)}{$token}{/if}');" title="{l s='remove this product from my wishlist' mod='fieldblockwishlist'}" rel="nofollow"><img src="/vela/food/modules/blockwishlist/img/icon/delete.gif" width="12" height="12" alt="{l s='Delete'}" class="icon" /></a>
				</dt>
		...
		{else}
			<dl class="products">
				<dt>{l s='No products' mod='fieldblockwishlist'}</dt>
			</dl>
		{/if}
		</div>
		<p class="lnk">
		...
		{/if}
			<a href="{$link->getModuleLink('fieldblockwishlist', 'mywishlist', array(), true)|addslashes}" title="{l s='My wishlists' mod='fieldblockwishlist'}" rel="nofollow">&raquo; {l s='My wishlists' mod='fieldblockwishlist'}</a>
		</p>
	</div>
</div>