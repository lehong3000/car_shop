...
class FieldThemeCustomizer extends Module
{
    ...
    private function _createConfigs()
    {
        ...
        $response &= Configuration::updateValue('FIELD_activeColorScheme', '#f28646');

        // Background Options
        $response &= Configuration::updateValue('FIELD_backgroundColor', '#ffffff');
        ...
        $response &= Configuration::updateValue('FIELD_bodyBackgroundColor', '#ffffff');
		...
		$response &= Configuration::updateValue('FIELD_breadcrumbBackgroundImage', '');
        // Custom Codes
    ...
    {
		 $bgImageDirs = _MODULE_DIR_ . $this->name . '/views/img/front/bg/';
        ...
        $this->_cssRules = array(
            ...
            'FIELD_titleFont' => array(
                array(
                    'selector' => '.button_unique,.popup_title h2, #search_filters > h4,#search_filters .facet .facet-title,.tabs .nav-tabs .nav-link,.title_font,h1,h2,h3,h4,h5,h6,.sale_product,.new_product,.horizontal_mode .quick-view,.product-actions .add-to-cart',
                    'rule' => 'font-family'
                )
            ),
        ...
        $this->_configDefaults = array(
            'FIELD_mainColorScheme' => '#292929',
            'FIELD_activeColorScheme' => '#f28646',
        ...
        // Web-safe Fonts
        $this->_websafeFonts = array('Roboto', 'Raleway', 'Helveticaneue', 'sans-serif');
	...
	//var_dump($productVerticalThumb);
        if($productVerticalThumb==1){
		$this->context->controller->registerJavascript('field-themes-jquery.carouFredSel-6.2.1.min', '/assets/field-js/jquery.carouFredSel-6.2.1.min.js', ['position' => 'bottom', 'priority' => 10000]);
        }
