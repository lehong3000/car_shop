...
{block name='page_content'}
    <div id="view_wishlist">
        <h1 class="page-heading bottom-indent">
            {l s='Wishlist' mod='fieldblockwishlist'}
        </h1>
        {if $wishlists}
            <p>
                <strong class="dark">
                    {l s='Other wishlists of %1s %2s:' sprintf=[$current_wishlist.firstname, $current_wishlist.lastname] mod='fieldblockwishlist'}
                </strong>
                {foreach from=$wishlists item=wishlist name=i}
                    {if $wishlist.id_wishlist != $current_wishlist.id_wishlist}
                        <a href="{$link->getModuleLink('fieldblockwishlist', 'view', ['token' => $wishlist.token])|escape:'html':'UTF-8'}" rel="nofollow" title="{$wishlist.name|escape:'html':'UTF-8'}">
                        ...
                        <div class="product_infos">
                            <p id="s_title" class="product-name">
                                ...
                                {if isset($product.attributes_small)}
                                    <a
                                        ...
                                        title="{l s='Product detail' mod='fieldblockwishlist'}">
                            ...
                            <div class="wishlist_product_detail">
                                ...
                                <p class="form-group selector1">
                                    <span><strong>{l s='Priority' mod='fieldblockwishlist'}:</strong> {$product.priority_name}</span>
                                </p>
                                <div class="button-action">
                                    {if !$configuration.is_catalog}
                                        ...
                                        {else}
                                            <span class="button btn-add-to-cart btn btn-default disabled">
                                                {if (isset($product.attribute_quantity) && $product.attribute_quantity >= 1)}
                                                    <i class="fa fa-shopping-cart"></i>
                                                {else}
                                                    <i class="fa fa-ban"></i>
                                                {/if}
                                        ...
                                        {/if}
                                    {/if}
                                    <a class="button lnk_view btn btn-default" href="{$link->getProductLink($product.id_product,  $product.link_rewrite, $product.category_rewrite)|escape:'html':'UTF-8'}" title="{l s='View' mod='fieldblockwishlist'}" rel="nofollow">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                </div>
                                <!-- .btn_action -->
