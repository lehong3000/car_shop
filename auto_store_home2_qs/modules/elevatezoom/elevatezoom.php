...
class ElevateZoom extends Module
{
	...
	public function __construct()
	{
		...
		$this->description = $this->l('Enables zooming on images and magnifier effects. Modded by Biren Mehta.');
	...
	public function install()
	{
		...
		return (parent::install() &&
			...
			Configuration::updateValue('ELEVATEZOOM_ZOOM_TYPE', 'window') &&
			...
			Configuration::updateValue('ELEVATEZOOM_CURSOR_TYPE', 'default') &&
			
			Configuration::updateValue('ELEVATEZOOM_TINT', 'true') &&
			Configuration::updateValue('ELEVATEZOOM_TINT_COLOR', '#333') &&
			Configuration::updateValue('ELEVATEZOOM_TINT_OPACITY', 0.4) &&
			
			Configuration::updateValue('ELEVATEZOOM_LENS_SHAPE', 'round') &&
			Configuration::updateValue('ELEVATEZOOM_LENS_SIZE', 345) &&
				
			Configuration::updateValue('ELEVATEZOOM_EXTRA_PARAMS', 'borderSize:0,borderColour:\'\', zoomWindowWidth:508, zoomWindowHeight:508, zoomLevel:0.5,lensBorderSize:0') &&
			...
			$this->registerHook('elevatezoom')&& 								
			$this->registerHook('displayHeader'));	//install and register the module on header hook
	}
	...
	public function uninstall()
	{
		...
		if (!parent::uninstall() ||
			...
			!$this->unregisterHook('elevatezoom') ||
...
// Check for parent tab2
        ...
        $tab = new Tab();
        $tab->active = 1;
        $tab->class_name = "AdminFieldElevatezoom";
        $tab->name = array();
        foreach (Language::getLanguages() as $lang){
            $tab->name[$lang['id_lang']] = "Config Elevatezoom";
        }
    ...
    {
        $id_tab = Tab::getIdFromClassName('AdminFieldElevatezoom');
		if($id_tab){
			$tab = new Tab($id_tab);
			$tab->delete();
		}
	...
	public function displayForm()
	{
		...
		$html .= '	
			<fieldset class="fieldset-credits">
				...
				<div>
					...
					<p>'.$this->l('Adapted to Prestashop 1.6.x.x by Biren Mehta.').'</p>	
		...
		public function hookElevatezoom($params)
	{	
		$this->smarty->assign(array(		
			'content_dir' =>$this->context->link->getMediaLink($this->_path),	
	...
	$smarty->assign('HOOK_ELEVATEZOOM', $this->hookElevatezoom($params));
